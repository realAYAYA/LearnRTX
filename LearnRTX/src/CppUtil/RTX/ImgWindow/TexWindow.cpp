...
TexWindow::TexWindow(const string & title)
	...
	auto pRootPath = config->GetStrPtr("RootPath");
	if (pRootPath == NULL) {
...
bool TexWindow::Run(Operation::Ptr texUpdateOp) {
	...
	//------------ Screen Shader
	string screen_vs = rootPath + "\\assets\\shaders\\RTX\\P2T2.vs";
	string screen_fs = rootPath + "\\assets\\shaders\\RTX\\T2.fs";
	...
	//------------ ppBlur Shader
	string ppBlur_vs = rootPath + "\\assets\\shaders\\RTX\\P2T2.vs";
	string ppBlur_fs = rootPath + "\\assets\\shaders\\RTX\\ppBlur.fs";
	...
	string interpolation_vs = rootPath + "\\assets\\shaders\\RTX\\P2T2.vs";
	string interpolation_fs = rootPath + "\\assets\\shaders\\RTX\\interpolation.fs";
	...
	//------------ Gamma Shader
	string gamma_vs = rootPath + "\\assets\\shaders\\RTX\\P2T2.vs";
	string gamma_fs = rootPath + "\\assets\\shaders\\RTX\\gamma.fs";
...
Ptr<Config> TexWindow::DoConfig() {
	...
	rootPath = string("C:\\Users\\AYAYA2\\source\\repos\\LearnRTX\\LearnRTX");
	...
	*pRootPath = rootPath;
	...
	GStorage<Ptr<Config>>::GetInstance()->Register("MainConfig", config);
	return config;
}