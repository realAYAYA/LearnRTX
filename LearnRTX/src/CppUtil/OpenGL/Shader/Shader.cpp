...
#include <GL/glew.h>
#include <GLFW/glfw3.h>//#include <GLFW/Glfw.h>
...
void Shader::SetFloats(const char* name, size_t n, const float * values) const {
	Use();
	glUniform1fv(glGetUniformLocation(ID, name), n, values);
}


void Shader::SetVec2f(const char* name, float value0, float value1) const {
	Use();
	glUniform2f(glGetUniformLocation(ID, name), value0, value1);
}

void Shader::SetVec2f(const char* name, const glm::vec2 & v) const {
	SetVec2f(name, v.x, v.y);
}

void Shader::SetVec3f(const char* name, float value0, float value1, float value2) const {
	Use();
	glUniform3f(glGetUniformLocation(ID, name), value0, value1, value2);
}

void Shader::SetVec3f(const char* name, const glm::vec3 & v) const {
	SetVec3f(name, v.x, v.y, v.z);
}

void Shader::SetVec3fs(const char* name, size_t n, const float * data) const {
	Use();
	glUniform3fv(glGetUniformLocation(ID, name), 4, data);
}

void Shader::SetVec4f(const char* name, float value0, float value1, float value2, float value3) const{
	Use();
	glUniform4f(glGetUniformLocation(ID, name), value0, value1, value2, value3);
}

void Shader::SetVec4f(const char* name, const glm::vec4 & v) const {
	SetVec4f(name, v.x, v.y, v.z, v.w);
}

void Shader::SetMat4f(const char* name, const float * matValue) const{
	Use();
	glUniformMatrix4fv(glGetUniformLocation(ID, name), 1, GL_FALSE, matValue);
}

void Shader::SetMat4f(const char* name, const glm::mat4& mat4) const{
	Use();
